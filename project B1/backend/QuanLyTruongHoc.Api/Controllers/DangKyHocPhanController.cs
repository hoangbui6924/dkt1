...
public class DangKyHocPhanController : ControllerBase
{
    ...
    private const decimal DiemHetCaiThien = 7.0m;
    ...
    public DangKyHocPhanController(AppDbContext db, IAiChatService aiChat)
    {
        _db = db;
        _aiChat = aiChat;
    }
    ...
    private static List<BuoiHocDto> ToBuoiHocs(IEnumerable<LichHocLopHocKy> lichs) =>
    ...
    private static bool TrungLich(IEnumerable<LichHocLopHocKy> a, IEnumerable<LichHocLopHocKy> b)
    {
        foreach (var x in a)
            foreach (var y in b)
                if (LopHocTrongKyController.BuoiTrungNhau(
    ...
    private static bool DotApDungCho(DotDangKy d, Domain.Entities.SinhVien sv)
    {
    ...
    private async Task<HocKy?> ResolveHocKyHienTaiAsync(Domain.Entities.SinhVien sv)
    {
        var now = VnNow();
    ...
    private (int namThu, int kyDat) TinhKyDat(HocKy hocKy, int namNhapHoc)
    {
    ...
    private async Task<ChuongTrinhDangKyDto> BuildChuongTrinhAsync(Domain.Entities.SinhVien sv, HocKy hocKy)
    {
    ...
    public async Task<ActionResult<ChuongTrinhDangKyDto>> GetChuongTrinh(int maHocKy)
    {
        ...
        return Ok(await BuildChuongTrinhAsync(sv, hocKy));
    }
    ...
    // =====================================================================
    // Gợi ý thời khoá biểu bằng AI (Trợ lý AI đọc yêu cầu tự do của sinh viên)
    // =====================================================================
...
- Nếu sinh viên không nêu yêu cầu gì đặc biệt cho một trường, để mảng rỗng hoặc giá trị null/KhongRo. Luôn trả lời bằng đúng 1 JSON hợp lệ.";
        ...
        Dictionary<int, LopHocTrongKy> daChon)
    {
        ...
        foreach (var (lop, score) in scored)
        {
            ...
            if (daChon.Values.Any(chosen => TrungLich(lop.LichHocs, chosen.LichHocs))) continue;
    ...
    public async Task<ActionResult<List<GoiYThoiKhoaBieuResultDto>>> GoiYThoiKhoaBieu(GoiYThoiKhoaBieuRequest request)
    {
        ...
        var hocKy = await ResolveHocKyHienTaiAsync(sv);
        if (hocKy is null) return BadRequest(new { message = "Hiện không có học kỳ nào để xếp lịch" });
        ...
        try
        {
            aiRaw = await _aiChat.ChatAsync(systemPrompt, request.YeuCau);
        }
        catch (Exception ex)
        {
            return StatusCode(502, new { message = "Không thể kết nối tới trợ lý AI: " + ex.Message });
        }
        ...
        foreach (var mon in chuongTrinh.MonHocs)
        {
            ...
            if (VanBanCoNhacTen(request.YeuCau, mon.TenMonHoc))
            ...
            if (constraints.SoMonTuChonBatKy > 0)
            {
                ...
                foreach (var mon in ungVien)
                {
                    ...
                    var lopPhuHop = candidates
                        ...
                        .FirstOrDefault(l => !daChonHienTai.Values.Any(c => TrungLich(l.LichHocs, c.LichHocs)));
                    if (lopPhuHop != null)
                    {
        ...
        var ketQuas = dsKetQuaXepLich.Select(XayDungKetQua).ToList();
        return Ok(ketQuas);
    }
}
