...
export default function KhoaVienPage() {
  ...
  const filtered = useMemo(() => {
    ...
    let result = items;
    if (q) {
      result = result.filter((i) => i.tenKhoaVien.toLowerCase().includes(q));
    }
    result = [...result].sort((a, b) =>
      sortDir === 'asc'
        ? a.tenKhoaVien.localeCompare(b.tenKhoaVien)
        : b.tenKhoaVien.localeCompare(a.tenKhoaVien),
    );
    return result;
            ...
            <tr className="border-b border-gray-200 bg-white">
              ...
              <th className="border-r border-gray-200 px-2 py-1">
                <div className="flex items-center gap-0.5 rounded border border-gray-200 bg-white px-1.5 py-0.5">
                  <input
                    type="text"
                    placeholder="→ Tìm theo tên khoa viện"
                    value={search}
                    onChange={(e) => {
