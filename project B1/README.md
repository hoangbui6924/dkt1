...
- **Chatbot hỏi–đáp (RAG, streaming)**: đọc tài liệu PDF đã tải lên **kết hợp** dữ liệu trong CSDL (cơ cấu tổ chức, môn học, giảng viên, độ khó môn học thống kê từ điểm khoá trước) để tư vấn; **có trí nhớ hội thoại** và biết từ chối câu hỏi ngoài phạm vi; trả lời **token sống (SSE)** + trích nguồn.
- **Chatbot agentic (tool-calling)**: tự gọi công cụ để **xếp lịch**, xem **lịch đã đăng ký** / **chương trình kỳ này**, **tìm web** khi cần thông tin ngoài hệ thống, và **đăng ký lớp học** — riêng đăng ký theo cơ chế an toàn: bot **đề xuất**, sinh viên **xác nhận** (chế độ "Chủ động có quy tắc", mặc định) hoặc cho bot **tự đăng ký** (chế độ "Trao quyền nguy hiểm").
